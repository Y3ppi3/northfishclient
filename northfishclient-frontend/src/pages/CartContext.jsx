...
export const CartProvider = ({ children }) => {
  ...
  const fetchCartItems = async () => {
    try {
      ...
      const res = await axios.get(`http://127.0.0.1:8000/cart/?_=${timestamp}`, {
  ...
  const addToCart = async (product_id, quantity = 1) => {
    try {
      ...
      await axios.post('http://127.0.0.1:8000/cart/', { product_id, quantity });
  ...
  const removeFromCart = async (cart_id) => {
    try {
      await axios.delete(`http://127.0.0.1:8000/cart/${cart_id}`);
      await fetchCartItems();
    } catch (err) {
  ...
  const updateCartItemQuantity = async (cart_id, quantity) => {
    try {
      ...
      await axios.put(`http://127.0.0.1:8000/cart/${cart_id}?quantity=${quantity}`);
      await fetchCartItems();
    } catch (err) {
